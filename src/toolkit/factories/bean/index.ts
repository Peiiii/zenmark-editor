...
const setNodeData = (
  ...
  const prevData = getNodeData(dataTree, path);
  if (value === prevData) return; // no change
  ...
  taskManager.freeze();
  console.log("setNodeData", path, value, prevData);
  if (!path) {
    dataTree.data = value;
    notify(controlTree, value, prevData);
  } else {
    const route = path.split("::");
    ...
    for (let i = 0; i < changedListWithNewData.length; i++) {
...
export const executeInit = (
  ...
  const renderingTasks = execution.renderingTasks;
  const cleanupTasks = execution.cleanupTasks;
  execution.renderingTasks = [];
  execution.cleanupTasks = [];
  return [
    result,
...
export const define = <O extends Record<string, any>, TInitArgs extends any[]>(
  ...
  //   const rawBean = createBean(getData(...args));
  ...
  //         methodName,
  //         (...args: Parameters<T2[T1]>) => {
  //           return fn(rawBean.get())[methodName](...args);
  //         },
  //       ])
  //     );
  //   };
